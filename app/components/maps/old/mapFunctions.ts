...
export const hideElement = (el: HTMLElement, focusEl?: HTMLElement) => {
  el.style.display = 'none';
  if (focusEl) focusEl.focus();
}

/** Shows a DOM element that has been hidden and optionally focuses on focusEl. */
export const showElement = (el: HTMLElement, focusEl?: HTMLElement) => {
  el.style.display = 'block';
  if (focusEl) focusEl.focus();
}

/** Determines if a DOM element contains content that cannot be scrolled into view. */
export const hasHiddenContent = (el: HTMLDivElement) => {
...
export const initializeMap = (configuration: any, widget: any, widgetEl: any) => { // TODO
  ...
  widget.map.addListener('click', (e:any) => { //todo
  ...
  widget.map.addListener('zoom_changed', () => {
    ...
    const hideDefaultPoiPins =
      widget.map.getZoom() < ND_DEFAULT_POI_MIN_ZOOM;
...
export const initializePlaceDetails = (widget: any) => { // TODO
  ...
  widget.fetchPlaceDetails = function (
    placeId: any,
    fields: any,
    callback: any
  ) {
    ...
    const missingFields = fields.filter(
      (field: any) => !place.fetchedFields.has(field) // TODO
    );
    ...
    let retryCount = 0;
    const processResult = function (result: any, status: any) {
      //TODO
      ...
      if (result.types) {
        ...
        for (let type of result.types) {
          if (type in ND_MARKER_ICONS_BY_TYPE) {
            ...
            place.icon = ND_MARKER_ICONS_BY_TYPE[type as MarkerTypes];
            break;
          }
        }
      }
...
export const initializeSidePanel = (widget: any, widgetEl: any) => { //todo
  ...
  const detailsTemplate = document.getElementById('nd-place-details-tmpl')!.innerHTML;
  const resultsTemplate = document.getElementById('nd-place-results-tmpl')!.innerHTML;

  // Show specified POI photo in a modal.
  const showPhotoModal = (photo: any, placeName: any) => { // TODO
    const prevFocusEl = document.activeElement as HTMLDivElement;
    ...
    photoModalEl.addEventListener('click', (e: any) => { //TODO
  ...
  let selectedPlaceId: any; //TODO
  widget.selectPlaceById = (placeId: any, panToMarker: any) => { //TODO
    if (selectedPlaceId === placeId) return;
    selectedPlaceId = placeId;
    const prevFocusEl = document.activeElement as HTMLDivElement;

    const showDetailsPanel = (place: any) => { //TODO
      detailsPanelEl.innerHTML = detailsTemplate; // detailsTemplate(place);
      ...
      detailsPanelEl.querySelectorAll('.photo').forEach((photoEl: any, i: number) => { // TODO
    ...
    const processResult = (place: any) => { //TODO
      if (place.marker) {
        widget.selectedPlaceMarker.setMap(null);
      } else {
  ...
  const renderPlaceResults = (places: any, startIndex: number) => { // TODO
    placeResultsEl.insertAdjacentHTML(
      'beforeend',
      resultsTemplate // resultsTemplate({ places: places })
    );
    placeResultsEl
      .querySelectorAll('.place-result')
      .forEach((resultEl: any, i: number) => { //TODO
        ...
        resultEl.querySelector('.name').addEventListener('click', (e: any) => { // TODO
          widget.selectPlaceById(place.placeId, /* panToMarker= */ true);
          e.stopPropagation();
        });
        resultEl.querySelector('.photo').addEventListener('click', (e: any) => { // TODO
  ...
  const showNextPlaces = (n: number) => {
    const nextPlaces = widget.places.slice(
      nextPlaceIndex,
      nextPlaceIndex + n
    );
    if (nextPlaces.length < 1) {
      hideElement(showMoreButtonEl as HTMLElement);
      return;
    }
...
export const initializeSearchInput = (widget: any, widgetEl: any) => { //TODO
